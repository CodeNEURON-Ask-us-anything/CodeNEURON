...
def verify_grounded_with_groq(claim: str, evidence_list: list, api_key: str = None):
    try:
        client = get_groq_client(api_key)
        if not client:
            return {"verdict": "NOT_ENOUGH_INFO", "confidence": 0.3, "explanation": "Groq API not configured."}
...
{{
    ...
    "explanation": "<short single sentence>"
        ...
        return {
            ...
            "explanation": result.get("explanation", "Groq assessment.")
        }
    except Exception as e:
        return {"verdict": "ERROR", "confidence": 0.0, "explanation": f"Groq failed: {str(e)}"}

def batch_verify_grounded_with_groq(claims_data: list, api_key: str = None):
    if not claims_data: return []
    try:
        client = get_groq_client(api_key)
        if not client:
            return [{"verdict": "NOT_ENOUGH_INFO", "confidence": 0.3, "explanation": "Groq API not configured."} for _ in claims_data]
        ...
        for i in range(len(claims_data)):
            ...
            if res:
                ...
                final_results.append({
                    ...
                    "explanation": res.get("explanation", "Groq assessment.")
                })
            else:
                final_results.append({"verdict": "NOT_ENOUGH_INFO", "confidence": 0.3, "explanation": "Omitted from response."})
        return final_results
    except Exception as e:
        return [{"verdict": "ERROR", "confidence": 0.0, "explanation": f"Groq failed: {str(e)}"} for _ in claims_data]
